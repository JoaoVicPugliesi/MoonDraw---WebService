...
export class IAssignCartOwnerUseCase {
  ...
  async execute({ 
    public_id 
  }: IAssignCartOwnerDTO): Promise<InvalidOwnerNotFoundErrorResponse | IAssignCartOwnerResponse> {

    const user: User | null = await this.iUserRepository.findUserById(public_id);
