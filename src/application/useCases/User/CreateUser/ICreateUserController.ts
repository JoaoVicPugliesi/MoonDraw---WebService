...
export class ICreateUserController {
    ...
    async handle(req: FastifyRequest, res: FastifyReply): Promise<void> {
        ...
        try {
            ...
            const useCaseResolve: boolean | User = await this.iCreateUserUseCase.execute(DTO);

            if(typeof useCaseResolve === 'boolean') {
                res.status(409).send({ message: 'Conflict: user with email provided already exists' });
            }
